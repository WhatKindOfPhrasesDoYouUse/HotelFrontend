...
const RoomAdministration = () => {
    ...
    useEffect(() => {
        const fetchHotelTypes = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5221/api/rooms/hotel/1');
    ...
    const handleDelete = async (roomId) => {
        const confirmDelete = window.confirm(`Вы действительно хотите удалить тип отеля с ID ${roomId}?`);
        if (!confirmDelete) return;

        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5221/api/rooms/${roomId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });

            setRooms(prev => prev.filter(ht => ht.id !== roomId));
            setSuccess(true);
    ...
    return (
        <div className="container">
            ...
            <div className="header">
                ...
                <h1>Управление списком комнат</h1>
            </div>
            ...
            {rooms.length > 0 ? (
                ...
                <div className="no-data">Нет данных о комнатах</div>
            )}
