...
const CardAdministration = () => {
    ...
    useEffect(() => {
        const fetchHotelTypes = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5221/api/cards', {
                    headers: {
    ...
    const handleDelete = async (cardId) => {
        const confirmDelete = window.confirm(`Вы действительно хотите удалить карту с ID ${cardId}?`);
        if (!confirmDelete) return;

        try {
            await axios.delete(`http://localhost:5221/api/cards/${cardId}` , {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                }
            });

            setCards(prev => prev.filter(ht => ht.id !== cardId));
            setSuccess(true);
    ...
    return (
        <div className="container">
            ...
            {cards.length > 0 ? (
                ...
                <div className="no-data">Нет данных о картах</div>
            )}
