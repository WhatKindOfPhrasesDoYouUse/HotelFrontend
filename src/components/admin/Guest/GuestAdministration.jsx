...
const GuestAdministration = () => {
    ...
    useEffect(() => {
        const fetchEmployeeTypes = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5221/api/guests', {
                    headers: {
    ...
    const handleDelete = async (guestId) => {
        const confirmDelete = window.confirm(`Вы действительно хотите удалить тип отеля с ID ${guestId}?`);
        if (!confirmDelete) return;

        try {
            await axios.delete(`http://localhost:5221/api/guests/${guestId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                }
            });

            setGuests(prev => prev.filter(ht => ht.id !== guestId));
            setSuccess(true);
                        ...
                        <table>
                            ...
                            {guests.map((guest) => (
                                <tr key={guest.id}>
                                    ...
                                    <td>{guest.client.surname}</td>
                                    <td>{guest.client.patronymic}</td>
                                    <td>{guest.client.email}</td>
                                    <td>{guest.client.phoneNumber}</td>
                                    <td>{guest.cityOfResidence}</td>
                                    <td>{guest.dateOfBirth.split('T')[0]}</td>
                                    <td>{guest.passportSeriesHash}</td>
                                    <td>{guest.passportNumberHash}</td>
                                    <td>
