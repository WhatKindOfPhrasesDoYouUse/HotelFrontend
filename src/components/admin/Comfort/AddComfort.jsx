...
const AddComfort = () => {
    ...
    const validateForm = () => {
        ...
        if (!/^[A-Za-zА-Яа-я\-]+$/.test(comfort.name)) {
    ...
    const handleChange = (e) => {
        ...
        setComfort(prev => ({ ...prev, [name]: value }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                name: comfort.name.trim(),
            };

            await axios.post("http://localhost:5221/api/comforts", payload, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                }
            });
            setSuccess(true);
            setTimeout(() => navigate("/comfort-administration"), 1500);
        } catch (err) {
            if (err.response?.data?.errors === 409) {
                setError("Такой комфорт уже существует");
            } else {
    ...
    return (
        <div className="container">
            ...
            <h1>Добавление комфорта</h1>

            <div className="card-form-card">
                {success && (
                    <div className="success-message">
                        <FaCheck /> Комфорт успешно добавлен!
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            placeholder="Например: бассейн, кондиционер и т.д."
