...
const EditBank = () => {
    ...
    useEffect(() => {
        const fetchComforts = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://localhost:5221/api/comforts/${comfortId}`, {
                    headers: {
                        authorization: `Bearer ${localStorage.getItem("token")}`
                    }});
                setComfort(response.data);
                setEditData({
                    name: response.data.name,
                });
            } catch (err) {
                console.error("Ошибка при загрузке комфорта:", err);
    ...
    const handleSave = async (e) => {
        ...
        try {
            await axios.patch(
                `http://localhost:5221/api/comforts/${comfortId}`, editData, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`
                    }
                });
