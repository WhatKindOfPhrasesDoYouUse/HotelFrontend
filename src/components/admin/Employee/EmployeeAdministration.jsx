...
const EmployeeAdministration = () => {
    ...
    useEffect(() => {
        const fetchEmployee = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5221/api/employees', {
                    headers: {
    ...
    const handleDelete = async (employeeId) => {
        const confirmDelete = window.confirm(`Вы действительно хотите удалить сотрудника с ID ${employeeId}?`);
        if (!confirmDelete) return;

        try {
            await axios.delete(`http://localhost:5221/api/employees/${employeeId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                }
            });

            setEmployees(prev => prev.filter(ht => ht.id !== employeeId));
            setSuccess(true);
                ...
                <div className="card">
                        ...
                        <table>
                            ...
                            {employees.map((employee) => (
                                <tr key={employee.id}>
                                    ...
                                    <td>{employee.client.surname}</td>
                                    <td>{employee.client.patronymic}</td>
                                    <td>{employee.client.email}</td>
                                    <td>{employee.client.phoneNumber}</td>
                    ...
                    <div style={{textAlign: 'right'}}>
                        <button onClick={() => navigate('/register-employee')}
                            ...
                            <FaPlus/> Зарегестрировать нового сотрудника
                        </button>
                    </div>
