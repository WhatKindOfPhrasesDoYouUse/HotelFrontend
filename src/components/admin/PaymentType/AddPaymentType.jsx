...
const AddPaymentType = () => {
    ...
    const validateForm = () => {
        ...
        if (!/^[A-Za-zА-Яа-я\-\s]+$/.test(paymentTypeData.name)) {
            errors.name = "Название может содержать только буквы, пробелы и дефисы";
            isValid = false;
        }
    ...
    const handleChange = (e) => {
        ...
        setPaymentTypeData(prev => ({ ...prev, [name]: value }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                name: paymentTypeData.name.trim(),
            };

            await axios.post("http://localhost:5221/api/payment-types", payload, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                }
            });
            setSuccess(true);
            setTimeout(() => navigate("/payment-type-administration"), 1500);
        } catch (err) {
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            placeholder="Например: карта, наличные и т.д."
