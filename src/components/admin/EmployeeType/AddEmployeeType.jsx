...
const AddEmployeeType = () => {
    ...
    const validateForm = () => {
        ...
        if (!/^[A-Za-zА-Яа-я\-]+$/.test(employeeType.name)) {
    ...
    const handleChange = (e) => {
        ...
        setEmployeeType(prev => ({ ...prev, [name]: value }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                name: employeeType.name.trim(),
            };

            await axios.post("http://localhost:5221/api/employee-types", payload);
            setSuccess(true);
            setTimeout(() => navigate("/employee-type-administration"), 1500);
        } catch (err) {
