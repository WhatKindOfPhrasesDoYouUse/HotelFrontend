...
const AdministrationHotelTypeList = () => {
    ...
    useEffect(() => {
        const fetchHotelTypes = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5221/api/hotel-types', {
                    headers: {
    ...
    const handleDelete = async (hotelTypeId) => {
        const confirmDelete = window.confirm(`Вы действительно хотите удалить тип отеля с ID ${hotelTypeId}?`);
        if (!confirmDelete) return;

        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5221/api/hotel-types/${hotelTypeId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });

            setHotelTypes(prev => prev.filter(ht => ht.id !== hotelTypeId));
            setSuccess(true);
            ...
            {hotelTypes.length > 0 ? (
                <div className="card">
                    <div className="table-container">
                        <table>
                            ...
                            {hotelTypes.map((hotelType) => (
                                <tr key={hotelType.id}>
                                    <td>{hotelType.id}</td>
                                    <td className="hotel-name">{hotelType.name}</td>
                                    <td className="description-cell">{hotelType.description}</td>
                                    <td className="actions-cell">
                                        <div className="actions">
                                            <button className="btn btn-delete"
                                                    onClick={() => handleDelete(hotelType.id)}>
                                                <FaTrash/> Удалить
                                            </button>

                                            <button
                                                onClick={() => navigate(`/edit-hotel-type/${hotelType.id}`)}
                                                style={{
                    ...
                    <div style={{textAlign: 'right'}}>
                        <button onClick={() => navigate('/add-hotel-type')}
                ...
                <div className="no-data">Нет данных об отелях</div>
            )}
...
export default AdministrationHotelTypeList;